...
def merge_splitted_translation(splitted_result, splitting_index):
    merged = splitted_result[1]
    for i in range(0, len(splitted_result) - 1):
        start_index = splitted_result[i+1].lower.find(speaker_with_index(splitting_index[i][1]))
...
def translate(driver, lang_from, lang_to, dialogue, timeout=600, max_len=5000):
    if translation_length(dialogue) < max_len:
        ...
        splitted_dialogue, splitting_index = split_text_with_overlap_v2(dialogue, max_len, overlap = 2500)
...
def convert_translation_to_save_format(translation, turns):
    convs = []
    start_index = translation.lower.find(speaker_with_index(0))
    for i in range(0, turns-1):
        speaker = speaker_with_index(i)
        next_start_index = translation.lower.find(speaker_with_index(i+1))
        utterance = {}
